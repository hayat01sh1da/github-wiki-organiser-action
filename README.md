...
jobs:
  organise-wiki:
    ...
    steps:
      - uses: hayat01sh1da/github-wiki-organiser-action@v1
        with:
          group-by: Owner
          language: English
...
- Pin the floating major (`@v1`) to receive fixes, or an exact tag / commit SHA where supply-chain rigour matters.
